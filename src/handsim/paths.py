...
def _env(name, default=None):
    """HANDSIM_* with the pre-rename OPPDEF_* names still honoured."""
    return os.environ.get(name, os.environ.get(name.replace("HANDSIM_", "OPPDEF_"), default))
from pathlib import Path

REPO = Path(__file__).resolve().parents[2]

#: vendored MuJoCo Menagerie (sparse clone: leap_hand, wonik_allegro, shadow_hand)
MENAGERIE = Path(_env("HANDSIM_MENAGERIE",
...
#: here; set HANDSIM_VEGA_URDF to your own copy.
...
VEGA_URDF = Path(_env("HANDSIM_VEGA_URDF") or _DEV_VEGA)

#: A URDF->MJCF compiler that strips the .glb visual meshes MuJoCo cannot decode.
#: Only needed alongside VEGA_URDF. Set HANDSIM_DEXTRACK to your own checkout.
_DEV_DEXTRACK = "/home/jetson3/projects/dextrack_vega"   # likewise
DEXTRACK = Path(_env("HANDSIM_DEXTRACK") or _DEV_DEXTRACK)
...
DATA = Path(_env("HANDSIM_DATA", REPO / "data"))
...
def compile_urdf(urdf: Path) -> str:
    ...
    from handsim.hands.f5d6 import repair_mjcf as _repair
    lock = Path(urdf).with_suffix(".handsim.lock")
    try:
        fh = open(lock, "w")
    except OSError:
...
def menagerie_xml(rel: str) -> str:
    ...
    """
    import re
    p = require(MENAGERIE / rel, "MuJoCo Menagerie model", "HANDSIM_MENAGERIE")
    xml = p.read_text()
