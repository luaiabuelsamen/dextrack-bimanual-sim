...
from handsim.envs.bimanual import BASE_HALF, BASE_DOF                       # noqa: E402
from handsim.control.expert import Expert, FINGER_JOINTS, _amp_vector, GRASP  # noqa: E402
...
def run_policy(seed, act_fn=None, mode="policy", n_steps=None, two_handed=True,
    ...
    lh_press = None
    from handsim.control.expert import palm_ctrl
...
def main():
    ...
    ap.add_argument("--data", default="legacy/results/bc_demos.npz")
    ap.add_argument("--out", default="legacy/results/bc.json")
    a = ap.parse_args()
    t0 = time.time()
