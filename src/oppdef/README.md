...
The package is still called `oppdef`, after the retracted "opposition deficit"
...
| was | is | why |
|---|---|---|
| `oppdef/retarget.py` | `grasping/retarget_pose.py` | single-pose retargeting under a choice of objective (keypoint vs ε); `human/retarget.py` is the trajectory version and imports it |
| `oppdef/track.py` | `track_core.py` | the reference-agnostic tracking core; `human/track.py` is the GRAB-specific tracker |
