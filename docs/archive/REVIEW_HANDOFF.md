...
4. **Improving learned policies on a task that can be solved by replay.**
   ...
   `rollout_expert` randomizes friction and mass but does not implement the XY-placement randomization described in its module introduction. See [learning/bc.py](../src/handsim/learning/bc.py). The recorded action-chunking null is consistent with a task already saturated by a one-step MLP, but is not evidence against action chunking generally.
...
2. **The Warp adapter multiplies per-world capacities by the world count.** In [vec.py](../src/handsim/vec.py), `WarpVec.__init__` passes `nconmax=nconmax * N` and `njmax=njmax * N`. The installed Warp `put_data` source defines both arguments per world; `naconmax` is the separate total-contact argument. Upstream documents the same [batch-size semantics](https://mujoco.readthedocs.io/en/stable/mjwarp/index.html#batch-sizes).
...
6. **Parity is narrower than its headline.** The current [warp_parity.py](../experiments/infra/warp_parity.py) imports `warp_fix` as a top-level module, although the packaged implementation is `handsim.sim.warp_fix`. Its Warp success calculation also omits the tilt clause used by the CPU expert. These are source-inspection findings; the GPU command was not rerun during this handoff.
...
Result: **56 passed in 46.76 s**. The default system interpreter did not have `handsim` installed, so the working CPU environment and explicit `PYTHONPATH` were used. Passing this suite does not cover the Warp-specific findings above. This documentation-only follow-up did not repeat the physics suite.

The replay diagnostic can be reproduced from the repository root with the same CPU interpreter:

```python
from handsim.learning.bc import rollout_expert, run_policy
...
which imports the removed `handsim.hands.axis.extremes` function. That test still
...
- **Task B's tilt is discarded.** `experiments/grasp_metrics/g3.py:29` puts its Y rotation in
  ...
  `src/handsim/grasping/task.py:89` and `:110` use only column 3, the X rotation. A direct
...
- **The scored outcome is position retention, with no orientation criterion.**
  `src/handsim/grasping/task.py:173` measures object position in the palm frame. The success
...
needed to run ε here at all — plausibly why `src/handsim/human/` never imported
