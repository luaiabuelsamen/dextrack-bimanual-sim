...
physical harness, where a pose that cannot be realised simply does not score.
    ...
    pose      the finger angles come from matching the human's fingertip
              geometry (keypoint retargeting, `handsim.grasping.retarget_pose`). The hand's
...
def keypoint_pose(hand_key, width):
    ...
    from handsim.grasping.retarget_pose import retargeter_for, transform_ref, KEYPOINT
...
def main():
    ...
    ap.add_argument("--out", default="legacy/results/pose_vs_wrench.json")
    a = ap.parse_args()
