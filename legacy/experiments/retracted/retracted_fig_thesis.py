"""RETRACTED 2026-09-13 -- do not run, do not cite.

This figure reported a McNemar p = 0.0010 on a "deficit cohort" of
{allegro 2.41 cm, f5d6 3.40 cm}. Both floors were artifacts of tracking
distal joint origins with the finger coupling dropped; corrected, the
four hands span 0.06-0.34 cm and no cohort exists. Its right panel also
plotted force-only data under a "wrench" axis label, and it hardcoded
the p-value and discordant counts instead of deriving them.

Kept only so the withdrawn figure can be traced to the code that made
it. See NOTES.md 2026-09-12 and 2026-09-13.
"""
raise SystemExit(__doc__)

# --- original source below, inert ---
...
# def main():
#     rows = []
#     for f in sorted(glob.glob("legacy/results/conf_*.json")):
...
#     ax = axes[1]
#     drows = []
#     for f in sorted(glob.glob("legacy/results/deficit_*.json")):
...
#     fig.tight_layout()
#     pathlib.Path("legacy/figures").mkdir(exist_ok=True)
#     fig.savefig("legacy/figures/thesis.png", dpi=170, bbox_inches="tight")
