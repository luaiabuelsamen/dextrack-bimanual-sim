...
WB=""; [ "${AUDIT_WANDB:-1}" = 1 ] && WB="wandb_activate=True wandb_project=dextrack-bimanual wandb_entity=luai-abuelsamen-university-of-california-berkeley wandb_name=$tag wandb_group=$inst"; export AUDIT_RUN_NAME=$tag
...
AUDIT_PEN_W=$w AUDIT_FORCE_W=0 AUDIT_PROBE_SPACING=$sp AUDIT_PROBE_SDF=$sdf timeout 7200 bash -c "$CMD $WB $EXTRA" > /workspace/dense/$tag.train.log 2>&1
...
for who in base ft; do
  ...
  AUDIT_ALLENV=1 AUDIT_PROBE_SPACING=0.002 AUDIT_SUMMARY=/workspace/dense/$tag.$who.summary.json AUDIT_LOG=/workspace/dense/$tag.$who.npy timeout 1800 bash -c "$ECMD" > /workspace/dense/$tag.$who.log 2>&1
