...
WB=""; [ "${AUDIT_WANDB:-1}" = 1 ] && WB="wandb_activate=True wandb_project=dextrack-bimanual wandb_entity=luai-abuelsamen-university-of-california-berkeley wandb_name=$tag wandb_group=scratch"; export AUDIT_RUN_NAME=$tag
...
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True timeout 14400 bash -c "$CMD $WB $EXTRA" > /workspace/scratch/$tag.train.log 2>&1
...
AUDIT_ALLENV=1 AUDIT_PROBE_SPACING=0.002 AUDIT_SUMMARY=/workspace/scratch/$tag.summary.json AUDIT_LOG=/workspace/scratch/$tag.npy timeout 1200 bash -c "$ECMD" > /workspace/scratch/$tag.eval.log 2>&1
