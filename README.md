...
| a policy trained on a real grasp | **not yet measured** | The run built to answer this seeded 7 of its 10 references with wrist offsets computed for a *different subject's* clip of the same name (a `seq`-keyed lookup; GRAB has 80 such collisions), so its seed labels are void and it is being repeated with subject-keyed seeds. What its end states do show: no policy tracked under 50 mm *and* ended holding the object un-buried; the two that tracked (5.7 mm, 34.6 mm) ended with the hand 13 mm and 9 mm inside the object; four ended at 0.00 mm penetration because the object was on the floor. From the one clean in-process comparison (`mug_drink_1`), PPO from the un-buried fit drops the mug from every start and from the buried start tracks at 22 mm ending 1330× weight. See [docs/STAGE2.md](docs/STAGE2.md) |
